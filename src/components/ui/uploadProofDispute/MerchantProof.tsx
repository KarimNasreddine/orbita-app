...
const MerchantProof: FC<MerchantProofProps> = ({ disputeId }) => {
  ...
  const checkUploadStatus = async () => {
    // Implement the logic to check the upload status for merchant
  };
    ...
    try {
      const response = await fetch(
        `/api/getSignedUrl?fileName=${encodeURIComponent(
          ...
          "merchant"
        )}&imageRole=${encodeURIComponent(formattedProofType)}`,
        {
          method: "GET",
          headers: {
    ...
    <div className="flex gap-12 shadow-[inset_0px_0px_10px_0px_#00000024] rounded-2xl p-8 items-center">
      ...
      <div className="grid grid-cols-4 grid-rows-2 text-white gap-4 text-center">
        {proofTypes.map((proofType, index) => {
          ...
          return (
            <div key={index} className="bg-[#6038d1] rounded-lg relative">
              ...
              <div className="flex flex-col items-center justify-between gap-2 px-10 py-5 h-full">
                ...
                <div className="flex justify-center items-center">
                  <label
                    className={cn(
                      "bg-[#5a41a0] opacity-100 p-4 rounded-lg mt-2 text-xs cursor-pointer relative overflow-hidden",
                  ...
                  >
                    ...
                    {loadingOrErrors[index] === "Loading" && (
                      <div className="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center">
                        <div className="animate-spin rounded-full h-8 w-8 border-t-2 border-b-2 border-[#aa92e6]"></div>
                      </div>
                    )}
